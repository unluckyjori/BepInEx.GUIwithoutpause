...
pub fn show(config: &mut Config, disclaimer: &mut Disclaimer, ctx: &Context) {
    const DISCLAIMER: &str = r#"The console is now disabled by default.
    ...
    For mod developers that like the old conhost console, you can enable it back by opening the BepInEx/config/BepInEx.cfg and setting to true the "Enables showing a console for log output." config option."#;
    ...
    Window::new("Disclaimer")
        ...
        .show(ctx, |ui| {
            ui.vertical(|ui| {
                ui.add(
                    Label::new(RichText::new(DISCLAIMER).font(FontId::proportional(20.0)))
                        .wrap(true),
                );
                ...
                if let Ok(elapsed_) = disclaimer.time_when_disclaimer_showed_up.unwrap().elapsed() {
                    ...
                    if can_close {
                        ...
                        ui.centered_and_justified(|ui| {
                            ui.label(
                                RichText::new(
                                    ((NEEDED_TIME_BEFORE_CLOSABLE + 1) - elapsed).to_string(),
                                )
