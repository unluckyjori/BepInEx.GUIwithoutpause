...
namespace Instances.Tests
{
    public class Tests
    {
        ...
        public void StaticFinishSuccessTest()
        {
            ...
            var processResult = Instance.Finish("dotnet", "--list-runtimes", delegate
            { outputReceived = true; });
        ...
        {
            var outputReceived = false;
            var processResult = Instance.Finish("dotnet", "run --project Nopes", delegate
            { outputReceived = true; });
            Assert.AreEqual(true, outputReceived);
        ...
        {
            var outputReceived = false;
            var processResult = await Instance.FinishAsync("dotnet", "--list-runtimes", default, delegate
            { outputReceived = true; });
            Assert.AreEqual(true, outputReceived);
            Assert.Zero(processResult.ExitCode);
        }
        ...
        {
            var outputReceived = false;
            var processResult = await Instance.FinishAsync("dotnet", "run --project Nopes", default, delegate
            { outputReceived = true; });
            Assert.AreEqual(true, outputReceived);
            Assert.NotZero(processResult.ExitCode);
        }
